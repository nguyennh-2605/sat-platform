...
// XỬ LÝ UPLOAD VÀ PARSE FILE
exports.parseDocument = async (file) => {
  if (!file) throw new ApiError(400, { error: 'Không tìm thấy file.' });
  ...
  if (file.mimetype === 'application/pdf') {
    ...
    throw new ApiError(400, { error: 'Chỉ hỗ trợ file PDF hoặc Word (.docx, .doc).' });
  }

  if (!rawText || !rawText.trim()) {
    throw new ApiError(400, { error: 'File rỗng hoặc không thể đọc được chữ từ file này.' });
  }

  const chunks = splitTextIntoChunks(rawText, 8000);

  // XỬ LÝ TUẦN TỰ (tạm nghỉ giữa các chunk để tránh bị Google block API)
