...
| Dashboard Home | `dashboard/default`, `dashboard/crm`, `dashboard/finance`, `dashboard/analytics` | `dashboard/academy`, `dashboard/productivity` | Role-dispatched; Admin uses real KPI/chart/attention/snapshot data plus a Studio-style AuditEvent Recent Activity table; Student follows Productivity's 9/3 task-and-calendar layout; Teacher uses an Academy-style action-first overview with class scope, deadline attention, compact schedule, 30-day class pulse, independently loaded learning insights, and evidence-based check-ins; visual QA pending |
| Authentication | `auth/v2` | `auth/v1` | Implemented; visual QA pending |
| Classroom list/detail | `dashboard/academy` | `dashboard/tasks` | Implemented with Lessons placement, Assignment/Test-only Activities, shared delivery composers, members, performance, and no local sticky page header. Assignment Detail now uses a role-aware shared shell, sticky Student work rail, and Teacher Student Work master-detail review queue with optional points/feedback |
| Classroom Lessons curriculum | `dashboard/infrastructure` and shared radix-nova primitives | Circle/Teachable curriculum hierarchy, Thinkific compact outline | Implemented as a role-aware Week → Session → content builder with one Week surface, compact content rows, client-local collapse preferences, contextual actions, and persisted accessible Week/Session reorder; mixed content reorder remains intentionally out of scope until it has one canonical ordering model |
| Admin Test Management / Teacher Test Library / Student Practice Center | `dashboard/tasks`, `dashboard/infrastructure` | `dashboard/file-manager`, `dashboard/default` | Admin uses platform-owned System Library and read-only Teacher Tests; Teacher Test Library manages content lifecycle only; Classroom Activities/Lessons own test delivery; Student attempt experience remains separate |
