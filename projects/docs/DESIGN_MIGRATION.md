...
| Dashboard Home | `dashboard/default`, `dashboard/crm`, `dashboard/finance`, `dashboard/analytics` | `dashboard/academy`, `dashboard/productivity` | Role-dispatched; Admin uses real KPI/chart/attention/snapshot data plus a Studio-style AuditEvent Recent Activity table; Student follows Productivity's 9/3 layout with unified draggable tasks, calendar, Next SAT, self-reported score goal, progress, practice picks, and compact recent results; Teacher retains its dedicated workspace home; visual QA pending |
