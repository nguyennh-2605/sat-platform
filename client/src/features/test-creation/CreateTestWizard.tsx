  ...
  return (
    <div className={ui.page}>
      <AppHeader
        ...
        subtitle={editTestId ? 'Update the structured content and review your changes' : 'Import, review, and organize SAT questions'}
      ...
      <main className="min-h-0 flex-1 overflow-y-auto">
        <div className={`${ui.content} flex min-h-full flex-col gap-6`}>
          {isLoadingEdit && <Card className="flex min-h-[360px] items-center justify-center gap-3 p-8 text-sm text-[#6B7280]"><LoaderCircle size={20} className="animate-spin text-[#1B7A5A]" />Loading exam content…</Card>}
              ...
              <Card className="flex min-h-[620px] flex-1 flex-col overflow-hidden border-[#C9D8D2]!">
                ...
                <div className="grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-2">
                  <section className="flex min-h-[520px] min-w-0 flex-col border-b border-[#C9D8D2] lg:border-b-0 lg:border-r">
                    ...
                    <textarea ref={editorRef} value={rawText} onChange={event => setRawText(event.target.value)} readOnly={editLocked} spellCheck={false} className="min-h-0 flex-1 resize-none bg-white p-4 font-mono text-xs leading-6 text-[#1A1A1A] outline-hidden placeholder:text-[#9CA3AF] read-only:cursor-not-allowed read-only:bg-[#F9FAFB] focus:bg-[#FCFEFD]" placeholder={activeImportTemplate} />
                  </section>
  ...
  return <div className="grid max-h-[65vh] gap-6 overflow-y-auto pr-1 md:grid-cols-[0.85fr_1.15fr]">
    <div>
      ...
      <ol className="mt-3 space-y-2.5 text-xs leading-5 text-[#4B5563]">
        <li><strong className="font-semibold text-[#1A1A1A]">1. Modules:</strong> start with <code className="rounded-sm bg-[#E8F5EF] px-1 py-0.5 text-[#145F47]">=== MODULE 1 ===</code>.</li>
        <li><strong className="font-semibold text-[#1A1A1A]">2. Questions:</strong> start each one with <code className="rounded-sm bg-[#E8F5EF] px-1 py-0.5 text-[#145F47]">QUESTION 1</code>.</li>
        <li><strong className="font-semibold text-[#1A1A1A]">3. Classification:</strong> add one <code className="rounded-sm bg-[#E8F5EF] px-1 py-0.5 text-[#145F47]">Domain:</code> and <code className="rounded-sm bg-[#E8F5EF] px-1 py-0.5 text-[#145F47]">Skill:</code> line.</li>
        <li><strong className="font-semibold text-[#1A1A1A]">4. Answers:</strong> use choices A–D for multiple choice, then add <code className="rounded-sm bg-[#E8F5EF] px-1 py-0.5 text-[#145F47]">Answer: B</code>. For student-produced responses, omit choices and enter the value.</li>
      </ol>
      {subject === 'MATH' && <div className="mt-4 rounded-lg border border-[#C9D8D2] bg-[#F2F8F5] px-3 py-2.5 text-xs leading-5 text-[#374151]"><strong className="font-semibold text-[#145F47]">Math uses strict LaTeX:</strong> write every question, choice, table cell and explanation as raw LaTeX without <code className="rounded-sm bg-white px-1 py-0.5">$</code> delimiters. Use <code className="rounded-sm bg-white px-1 py-0.5">\\text{'{'}...{'}'}</code> for words. Module markers, Domain, Skill and Answer lines remain plain structured metadata.</div>}
      ...
      <div className="mt-3 space-y-2 text-xs leading-5 text-[#4B5563]">
        <p><code className="rounded-sm bg-[#E8F5EF] px-1 py-0.5 font-semibold text-[#145F47]">[TEXT]</code> A passage or stimulus. Plain untagged passage text is also accepted.</p>
        <p><code className="rounded-sm bg-[#E8F5EF] px-1 py-0.5 font-semibold text-[#145F47]">[TABLE]</code> Put each row on a new line, separate cells with the <strong className="font-semibold text-[#1A1A1A]">Tab key</strong>, and use the first row as the header.</p>
        <p><code className="rounded-sm bg-[#E8F5EF] px-1 py-0.5 font-semibold text-[#145F47]">[POEM]</code> Put each verse line on its own line.</p>
        <p><code className="rounded-sm bg-[#E8F5EF] px-1 py-0.5 font-semibold text-[#145F47]">[NOTE]</code> Put each supporting fact on a new line, optionally beginning with <code>-</code> or <code>•</code>.</p>
        <p><code className="rounded-sm bg-[#E8F5EF] px-1 py-0.5 font-semibold text-[#145F47]">[IMG]</code> Put a publicly accessible image URL on the following line. The Upload image button inserts this block automatically.</p>
      </div>
    ...
    <div className="min-w-0">
      <div className="mb-2 flex items-center justify-between gap-3"><h3 className="text-xs font-semibold uppercase tracking-[0.08em] text-[#145F47]">Example template</h3><button type="button" onClick={() => void navigator.clipboard.writeText(template).then(() => toast.success('Template copied'))} className="text-xs font-semibold text-[#1B7A5A] hover:text-[#145F47]">Copy template</button></div>
      <pre className="max-h-[56vh] overflow-auto whitespace-pre-wrap rounded-lg border border-[#D5E5DF] bg-[#F9FCFA] p-4 font-mono text-[11px] leading-5 text-[#374151]">{template}</pre>
    </div>
  </div>;
}
