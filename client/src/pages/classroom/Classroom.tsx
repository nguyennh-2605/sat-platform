...
import { Button, Card, Input, Modal, PageHeader, Tabs, type TabItem } from '../../components/ui/AppUI';
...
function AnnouncementsTab({ classId, selectedAnnouncementId, refreshKey, canManage, onNewAnnouncement }: { classId: string; selectedAnnouncementId: string | null; refreshKey: number; canManage: boolean; onNewAnnouncement: () => void }) {
  ...
  return <div className="space-y-4 py-2">
    ...
    {loading ? <DataSurface className="h-40 animate-pulse bg-muted/40" /> : error ? <DataSurface className="p-8 text-center"><p className="text-sm font-medium text-foreground">Unable to load announcements</p><p className="mt-1 text-xs text-muted-foreground">{error}</p><Button className="mt-4" variant="outline" size="sm" onClick={() => void load()}>Try again</Button></DataSurface> : announcements.length === 0 ? <DataSurface className="flex min-h-56 flex-col items-center justify-center p-8 text-center"><Megaphone size={22} className="text-muted-foreground" /><h3 className="mt-3 text-sm font-semibold text-foreground">No announcements yet</h3><p className="mt-1 text-xs text-muted-foreground">Class updates from the teacher will appear here.</p></DataSurface> : <DataSurface>{announcements.map(item => <article key={item.id} id={`announcement-${item.id}`} className={`border-b border-ui-border/60 p-5 transition-colors last:border-0 hover:bg-muted/30 ${selectedAnnouncementId === item.id ? 'bg-muted/60 ring-2 ring-inset ring-primary/35' : ''}`}><div className="flex min-w-0 items-start gap-3"><span className="flex size-9 shrink-0 items-center justify-center rounded-control bg-primary/10 text-primary"><Megaphone size={16} /></span><div className="min-w-0 flex-1"><div className="flex flex-wrap items-start justify-between gap-2"><div><h3 className="text-sm font-medium text-foreground">{item.title}</h3><p className="mt-0.5 text-xs text-muted-foreground">{item.author.name || 'Teacher'} · {formatDistanceToNow(new Date(item.createdAt), { addSuffix: true })}</p></div></div>{item.content && <p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-muted-foreground">{item.content}</p>}{(item.fileUrls.length > 0 || item.links.length > 0) && <div className="mt-4 flex flex-wrap gap-2">{[...item.fileUrls, ...item.links].map(url => <a key={url} href={url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 rounded-control border border-ui-border px-2.5 py-1.5 text-xs font-medium text-foreground hover:bg-muted"><ExternalLink size={12} />Open link</a>)}</div>}</div></div></article>)}</DataSurface>}
  </div>;
}
...
function MemberTable({ label, members, canManage, onRemove }: { label: 'Teacher' | 'Student'; members: ClassMember[]; canManage: boolean; onRemove?: (student: ClassMember) => void }) {
  return <DataSurface><Table className="min-w-[680px] table-fixed">
    <TableHeader><TableRow><TableHead className="w-[30%]">{label}</TableHead><TableHead className="w-[34%]">Email</TableHead><TableHead className="w-[24%]">Joined</TableHead><TableHead className="w-[12%] text-right">Actions</TableHead></TableRow></TableHeader>
    <TableBody>{members.length === 0 ? <TableRow className="hover:bg-transparent"><TableCell colSpan={4} className="h-40 text-center text-muted-foreground">No students have joined this class yet.</TableCell></TableRow> : members.map((member, index) => <TableRow key={member.id}>
      <TableCell><div className="flex min-w-0 items-center gap-2.5"><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs font-semibold text-white" style={{ backgroundColor: AVATAR_COLORS[index % AVATAR_COLORS.length] }}>{initials(member.name)}</span><span className="truncate font-medium text-foreground">{member.name || `Unknown ${label.toLowerCase()}`}</span></div></TableCell>
      <TableCell className="truncate text-muted-foreground" title={member.email}>{member.email}</TableCell>
      <TableCell className="text-muted-foreground">{formatMemberSince(member.createdAt)}</TableCell>
      <TableCell className="text-right">{canManage && onRemove ? <button type="button" onClick={() => onRemove(member)} className="app-icon-button ml-auto h-8 w-8 text-muted-foreground hover:bg-destructive/10 hover:text-destructive" aria-label={`Remove ${member.name || 'student'}`} title="Remove student"><Trash2 size={15} /></button> : <span className="text-muted-foreground">—</span>}</TableCell>
    </TableRow>)}</TableBody>
  </Table></DataSurface>;
}
