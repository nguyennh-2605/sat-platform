...
function QuizWorkspace({ set, activityId, config, onBack }: { set: SetDetail; activityId?: string; config: QuizConfig | null; onBack: () => void }) {
  ...
  return <div className={ui.page}>
    <AppHeader title="Vocabulary Quiz" subtitle={set.title} rightContent={<SatCountdown />} />
    <main className="relative min-h-0 flex-1 overflow-y-auto"><LoadingBar active={loading || saving} /><BackButton label="Exit Quiz" onClick={onBack} className="absolute left-4 top-5 z-10 sm:left-6 lg:left-8" /><div className={`${ui.content} !max-w-[980px] !pt-5`}>
...
function QuizCompleteScreen({ session, elapsedSeconds, retrying, onRetry, onBack }: { session: StudySession; elapsedSeconds: number; retrying: boolean; onRetry: () => void; onBack: () => void }) {
  ...
  return <div className={ui.page}><AppHeader title="Quiz complete" subtitle="Vocabulary practice results" rightContent={<SatCountdown />} /><main className="relative min-h-0 flex-1 overflow-y-auto"><QuizConfetti /><div className={`${ui.content} relative !max-w-[1100px] py-10`}>
