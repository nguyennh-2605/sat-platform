...
export default function WorkspaceOverview({ role }: { role: 'TEACHER' | 'STUDENT' }) {
  const fallbackName = role === 'TEACHER' ? 'Teacher' : 'Student';
  const userName = localStorage.getItem('userName') || fallbackName;
  const firstName = userName.trim().split(/\s+/)[0] || fallbackName;
  ...
  const roleDestinations = destinations.map(destination => destination.to === '/dashboard/practice-test' && role === 'TEACHER'
    ? { ...destination, title: 'Test Library', description: 'Create and manage tests for your classes.' }
    : destination);
